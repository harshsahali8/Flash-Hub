...
export const createCheckoutSession = async ({ email }: { email: string }) => {
  try {
    const session = await stripe.checkout.sessions.create({
      ...
      mode: "subscription",
      success_url: `https://flsah-hub.vercel.app/dashboard/new-product`,
      cancel_url: `https://flash-hub.vercel.app/`,
    });
  ...
  try {
   ...
   if (!customer || !customer.id) {
    ...
    const portal = await stripe.billingPortal.sessions.create({
      ...
      return_url: `https://flash-hub.vercel.app/dashboard/my-products`,
    });
